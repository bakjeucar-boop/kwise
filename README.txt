...
화면 (Streamlit)
     ...
     산출물은 두 가지입니다.
       ...
       PPT     한 장에 한 가지만 담은 슬라이드입니다. 의사결정자용이며
               켠 수단만 한 장씩 들어갑니다

     Word 보고서는 만드는 코드가 남아 있으나 화면에서는 감췄습니다 (36세션).
     되살리는 방법은 src\kwise\ui\views\compare.py 의 주석에 있습니다.

     업로드한 파일은 서버에 저장하지 않습니다. 내려받기용 산출물도 만든 즉시
     지웁니다. 앱을 켤 때 묵은 임시 폴더를 쓸어냅니다.

     화면의 [자세히] 링크는 매뉴얼의 해당 절로 갑니다.
     문서를 고쳤으면 다시 만드십시오 - html 은 생성물입니다.
...
시험
     ...
     1,358건이 5분대에 끝납니다 (순차는 13분입니다).
     --dist loadfile 은 쓰지 마십시오 - 무거운 단일 파일을 못 쪼갭니다.
     -n auto 는 이 PC(6코어·RAM 7.4GB)에서 9%만 빠르고 메모리가 빠듯해집니다.
     갈래를 나눠 돌리는 법은 PROCEED.md 「pytest 분할 실행」에 있습니다.
...
docs\MANUAL.html             사용 매뉴얼 - 설계·영업 담당자용 (원본 MANUAL.md)
docs\TECHNICAL.html          기술서 - 개발자용 (원본 TECHNICAL.md)
docs\ENVIRONMENT.md          Windows 11 실행 환경
docs\REQUIREMENTS_kwise.md   요구사항서
docs\MANUAL_ANCHORS.md       매뉴얼 앵커 목록 (생성물)
docs\CAPTURES.md             화면 캡처 필요 목록
