"""역률 개선 (요구사항서 7.4) — 투자비가 작다 (콘덴서·APFR).

**콘덴서 조정만으로 기본요금이 줄어든다.** 약관 제43조 ②는 주간(08~22시)
지상역률이 기준 92%를 넘으면 매 1%당 기본요금의 0.2%를 감액한다. 감액 상한인
97%까지 올리면 **기본요금의 1.0%** 다. 기본요금이 연 4.5억 원인 샘플에서
450만 원이고, 진상용 콘덴서 증설비는 보통 그보다 훨씬 작다.
...
**콘덴서 과투자의 결과**로만 생긴다. 주간 97%를 맞추려 고정 콘덴서 단일 뱅크를
키우면 부하가 줄어든 야간에 진상으로 넘어가 되돌려 주게 된다.
**자동역률조정장치(APFR)** 는 부하에 따라 투입 단수를 조절해 이를 피한다.
...
def evaluate_power_factor(
    ...
    *,
    ...
    """콘덴서 조정으로 역률을 올렸을 때의 절감액을 **재계산해서** 낸다.
    ...
    warnings.append(
        "**자동역률조정장치(APFR)를 쓰면 부하에 따라 투입 단수가 조절되어 야간 "
        "진상을 피할 수 있습니다.** 고정 콘덴서 단일 뱅크로 주간 97% 를 맞추면 "
