...
class DemandResponseResult:
    ...
    Attributes:
        reduction_kw: 감축계획량으로 삼은 용량. 진단의 보수적 등록 가능 용량이다.
        max_reduction_kwh: **전 거래일 참여**를 가정한 상한. 실제로는 낙찰된 날만이다.
...
def evaluate_demand_response(
    ...
    *,
    ...
    capacity = profile.registrable_kw if reduction_kw is None else reduction_kw
    if capacity < 0:
        raise ValueError(f"감축계획량은 음수일 수 없습니다: {capacity}")

    # 감축량은 **거래 가능일 기준으로만** 낸다 (제12.4.2.1조 제1항 1호).
    max_kwh = capacity * bid_hours_per_day * profile.eligible_days
    low_cost_kwh = capacity * bid_hours_per_day * len(profile.low_load_days)

    settlement = None if unit_price_won_per_kwh is None else max_kwh * unit_price_won_per_kwh
    low_cost_settlement = (
    ...
    notes = [
        ...
        f"감축계획량 {capacity:,.0f} kW 는 진단의 **보수적** 등록 가능 용량입니다 "
        f"(대상일 주간 부하 하위 10% − 기저부하). 평균 기준 여력은 "
        f"{profile.mean_reducible_kw:,.0f} kW 지만 그 값으로 등록하면 절반의 날에 "
        "미달합니다.",
