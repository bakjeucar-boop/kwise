...
def min_pcs_power_kw() -> float:
    ...
    필요 출력이 이보다 작으면 **필요보다 큰 설비를 사야 하므로 산출하지 않는다.** 5~20 kWh
...
BASE_FEE_ON_CONTRACT_CONCLUSION = (
    "{label} 은 기본요금을 계약전력으로 매깁니다 (기본공급약관 제68조 제2항). "
    "피크를 낮춰도 기본요금이 줄지 않아 피크저감 목적의 ESS 는 성립하지 않습니다."
)
"""**계약전력 기준 종별의 결론** (56세션 · 61세션에 범위를 좁혔다).

기본요금이 **요금적용전력**에 붙으면 피크를 낮추는 만큼 준다. **계약전력**에
붙으면 아무리 깎아도 그대로다 — 남는 것은 충·방전 단가차뿐인데, 갑Ⅰ·교육용(갑)은
전력량요금이 **단일 단가**라 그 차이도 0 이고 왕복손실만 남는다.
**절감액이 음수**가 된다.
...
0 이하를 만나고, 후보가 비어 창만 상한까지 넓히고 끝난다. 56세션 전에는 그
자리에서 **없는 목표를 내고 절감액이 −3,000원인 카드**가 나왔다.
"""

BELOW_MINIMUM_CONCLUSION = (
    "필요 출력이 {power:,.0f} kW 인데 상업용 ESS 의 최소 규격은 {minimum:,.0f} kW 입니다. "
    "필요보다 큰 설비를 사야 하므로 산출하지 않았습니다."
)
"""필요 출력이 최소 규격에 못 미칠 때의 **결론** (50세션 3-3 · 63세션에 다시 썼다).

**주어가 시장이 아니라 이 건물이다.** 50세션의 첫 판은 「이 규모에 맞는 상업용
제품을 찾기 어려워」 였는데, 62세션 조사에서 그 문장이 **「제품만 나오면 되겠다」
로 읽힌다**는 것이 드러났다. 사실은 그 반대다 — 이 건물이 깎을 피크가 설비
한 대보다 작아서, 놓으면 **필요보다 큰 것을 사게 된다.**

**회수기간을 사유로 쓰지 않는다** (63세션). 62세션이 문지기를 끄고 재 보니
68.6년이었지만 초기 검토 단계에서는 투자비·수명·운전 전략이 미확정이라
그 수를 배제 사유로 화면에 낼 만큼 단단하지 않다. 배제 사유는 **제품 규격**
하나이고, 재 본 회수기간은 `docs\\BILL_CHECK.md` 에 남긴다.

**확인되지 않은 판정을 적지 않는다.** 「경제성 없음」 이라 쓰지 않는다.
회수기간과 목표별 사양 표를 싣지 않고, **필요 출력·용량·방전시간은 낸다.**
그것은 이 건물의 사실이다.
