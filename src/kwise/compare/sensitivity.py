...
# **「벌어지는 것은 피크에 걸린 기본요금 절감액입니다」 를 걷었다** (S170 2절). 기본요금이
# 피크에 안 매이는 벌(계약형 넷 · 하한이 전 달에 걸린 셋)에서는 기본요금 절감 폭이
# 0원이라 거짓이었다. 「때문입니다」 가 받던 첫 문장 곁으로 옮겨 붙였다. 「전력량요금
# 절감액은 세 시나리오에서 거의 같습니다」 도 걷었다 — `small-ind-a1` 에서 그 폭이
# 값의 13.7% 였다.
SENSITIVITY_NOTE = (
    "감도는 태양광 출력의 **첨예도**만 조정합니다. 발전량 예측의 불확실성이 총량이 "
    "아니라 정오 첨예도에 있기 때문입니다. "
    "adjusted(t) = 일평균발전 + (기준(t) − 일평균발전) × s 이므로 일별 총 발전량이 "
    "보존됩니다. "
    "요금제 전환·계약전력 조정·역률 개선은 "
    "실측 데이터와 요금표만으로 확정되는 계산이라 감도를 적용하지 않습니다. "
    "확률 표기(P90 등)는 쓰지 않습니다."
)

# 이름을 오해하기 쉬운 지점이다. 반드시 함께 낸다.
# 「첨예형의 기본요금 절감액이 크고」 를 kW 로 옮겼다 (S170 2절) — 기본요금이 피크에
# 안 매이는 벌에서는 세 시나리오의 기본요금 절감액이 같다.
SCENARIO_NAME_CAVEAT = (
    "시나리오 이름은 **발전 프로파일의 모양**을 가리킵니다. 좋고 나쁨의 축이 "
    "아닙니다. '첨예형'은 정오 출력을 더 뾰족하게 본다는 뜻이며, 일별 총량이 "
    "보존되므로 그만큼 아침·저녁 어깨 시간대 출력이 낮아집니다. 부하 피크가 정오에 "
    "있으면 첨예형이 피크를 더 낮추고, 정오에서 벗어나 있으면(오전 피크형·"
    "오후 피크형) 평탄형이 더 낮춥니다. **어느 쪽이 최댓값인지는 건물 부하 형태가 "
