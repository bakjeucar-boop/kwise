...
합계 행은 **「합계」** 다 (S244 결정 2). 수단을 함께 도입하면 서로 영향을 주므로
...
# 「단순 합」 은 3단계에서 합산효과와 맞대는 수(체크한 수단의 합) 하나다 — 켠 수단
# 전부의 합계는 그 이름으로 부르지 않는다 (S244 결정 2).
SIMPLE_SUM_LABEL = "합계"
SIMPLE_SUM_NOTE = (
    "**합계는 최종 효과가 아닙니다.** 수단을 함께 도입하면 서로 영향을 주므로 "
    "합산효과에서 조합을 통째로 다시 계산합니다."
)
...
def standalone_frame(rows: tuple[StandaloneRow, ...]) -> pd.DataFrame:
    """3단계 화면의 요약 표. **합계 행에 「합계」 라고 적는다** (S244 결정 2).
    ...
    **확실성 열은 없다** (28세션 4절 · 53세션 1-4 에 산출물도). 무엇에 대한 등급인지가 이름에 없어
