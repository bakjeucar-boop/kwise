...
들어갔는지 알 수 없다. **둘을 같은 줄에 둔다.**
    ...
    기본요금   월평균 5,226.276 kW × 8,320 원/kW × 12.00개월   521,791,000원
...
def _base_fee_row(bill: BillingResult, label: str = "역률 조정 전 기본요금") -> WorkRow:
    ...
    **곱하는 것은 최대가 아니라 월평균이다.** 기본요금은 달마다 그 달의 기준
    ...
    를 적어 곱이 늘 총액보다 컸다 — 대형 실측에서 5,780,963.75원(1.3%)이라
    이 표를 읽고 검산하면 어긋났다. 값을 만드는 자리는
    :attr:`~kwise.tariff.BillingResult.mean_base_demand_kw` 하나다.

    **「요금적용전력」 이라 부르지 않는다.** 계약전력 기준 종별(갑Ⅰ·교육용(갑)
    저압 · 제68조 ②)에서는 기본요금을 계약전력이 매기므로 그 이름이 거짓이
    된다 — `large-a` 에서 66,571,213원 차다. 이름은 이미 서 있는 것을 쓴다
    (``columns.py`` 의 ``base_demand_kw`` = 「기본요금 기준전력(kW)」).

    **kW 만 소수 셋째 자리까지 적는다.** 다른 kW 표기는 `,.1f` 인데 이 값은
    가중평균이라 나눗셈에서 나온다 — `,.1f` 로 접으면 값 칸(천 원 절사)과
    2,004~2,888원 어긋나 **읽고 검산하면 또 실패한다.** 덱 벌 열넷의 스물여섯
    줄에서 `,.3f` 는 천 원 자리까지 다 맞았다.
    """
    return WorkRow(
        label,
        # **한 줄로 적는다** — 줄을 가르면 「기본요금 산식 문장」 그물이
...
def contract_worksheet(result: ContractAdjustment) -> Worksheet:
    ...
    **총액 둘의 차이**다. 산식 칸에 앞엣것만 적어 두면 종별 갈래에서
    ...
    if result.crosses_type:
        ...
        # 아래 「두 줄의 차가 절감액이 아니다」 가 역률요금 절감 줄의 까닭이다.
        #
        ...
        formula = "현재 − 조정 후"
        if power_factor_cut:
            rows.append(WorkRow("기간 역률요금 절감", "기본요금이 줄면 함께 준다", _won(lines[2])))
...
def ess_worksheet(result: EssResult) -> Worksheet:
    ...
    rows = [
        ...
        WorkRow(
            "필요 용량",
            "정격 = 내보낼 에너지 ÷ √왕복효율 ÷ DoD",
            ess_capacity_text(result.required_capacity_kwh or result.capacity_kwh),
        ),
