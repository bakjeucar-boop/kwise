...
def terms(pattern: LoadPattern | None = None) -> dict[str, Term]:
    ...
    **시간대 경계는 자료가 정한다** — :class:`LoadPattern` 이 쓴 값을 그대로
    ...
    # 기준을 말하는 문장은 요금 엔진의 요금적용전력 안내가 따로 낸다.
    return {
        "load_factor": Term(
            "부하율",
            "평균 수요 ÷ 최대 수요.",
            "낮을수록 최대 수요가 평균 수요보다 높이 솟은 모양입니다.",
            "평균 수요 ÷ 최대 수요",
        ),
        ...
        "billing_demand": Term(
            "요금적용전력",
            "직전 12개월 최대수요. 하한이 걸리면 하한입니다.",
            "경부하 시간대의 피크는 대상이 아닙니다.",
            "직전 12개월 최대수요 (하한이 걸리면 하한)",
        ),
        "midday_share": Term(
            "상위 구간 정오 비중",
            "최대수요 상위 구간 가운데 정오 시간대에 든 비율.",
            "높을수록 태양광이 피크를 낮출 여지가 큽니다.",
            "상위 구간 중 정오 시간대 비율",
        ),
        "weekend_slot_share": Term(
            "상위 구간 주말 비중",
            "최대수요 상위 구간 가운데 토·일·공휴일에 든 비율.",
            "높으면 쉬는 날의 몇십 분이 1년 최대수요를 정한 것입니다.",
            "상위 구간 중 토·일·공휴일 비율",
        ),
        "base_fee": Term(
            "기본요금",
            # **역률요금 항을 세운다** (S142 1절). 「기본요금」 은 역률 가감이
            # 반영된 뒤의 금액이라고 S141 이 정했으므로, 이 항이 없으면 각주가
            # 큰 글자를 검산하지 못한다 — `large-b-pf85` 에서 산식값
            # 452,804,556 원 대 큰 글자 459,143,820 원으로 6,339,264 원 갈렸다.
            # **역률요금이 0원인 벌에서도 참이다** (간주 92% — 0을 더한다).
            "월평균 기본요금 기준전력 × 단가 × 개월수 + 역률요금.",
...
#: **시각만 말한다** (S170 2절). 앞서는 높음·보통이 「태양광이 기본요금을 줄일
#: 여지」 라는 원의 결론을 적어, 기본요금이 피크에 안 매이는 벌(계약형 넷 ·
#: 하한형 둘)에서 거짓이었고 7장 「몫이 작습니다」 와 열세 벌에서 부딪쳤다 —
#: S169 가 4장에서 걷은 것과 같은 꼴이다. 낮음 갈래의 거울 문장으로 맞췄다.
_PV_LEAD: dict[PvPotential, str] = {
    PvPotential.HIGH: ("피크의 {share}가 낮 시간에 발생해 태양광 발전 시간과 겹칩니다."),
    PvPotential.MEDIUM: ("피크의 {share}가 낮 시간에 발생해 태양광 발전 시간과 일부 겹칩니다."),
...
def structure_lead(structure: ChargeStructure) -> str:
    ...
    # **세 갈래 모두 수단을 이름하지 않는다** (S170 2절). 「최대수요를 낮추는」 ·
    # 「피크 저감」 · 「피크를 낮춰도」 는 피크형만의 길이다 — 계약형(`small-ind-a1` ·
    # `large-a` …)·하한형(`small-a2-was` · `large-b-over` …)에서 기본요금을 줄이는
    # 길은 계약전력이다. 비중만 보는 이 문장은 갈래를 모르므로 「무엇을」 까지만 말한다.
    if share > base_fee_share_high():
        return f"기본요금이 {_pct(share)}로 큽니다 — 기본요금을 줄이는 방안을 먼저 검토합니다."
    if share >= base_fee_share_low():
        return (
            f"기본요금 {_pct(share)}와 전력량요금이 함께 큽니다 — "
            "두 요금을 같이 줄이는 방안을 검토합니다."
        )
    ...
    return (
        f"전체 요금 중 기본요금은 {_pct(share)}입니다 — "
        "기본요금을 줄여도 전체에서 줄어드는 몫이 작습니다."
    )
