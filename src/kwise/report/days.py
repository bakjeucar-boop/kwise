...
세 그림을 나란히 놓고도 견줄 수 없다. 기준을 여기 한 곳에서 정한다.
    ...
    기간 최대수요일  기본. 피크가 어떻게 생겼는지가 세 수단의 공통 관심사다
...
def representative_days(usage: UsageData) -> tuple[RepresentativeDay, ...]:
    ...
    days = [
        RepresentativeDay(
            key=MAX_DEMAND_KEY,
            # **「기간 최대수요일」 이다** (S213 · 요구사항서 5.5). 아래 `reason`
            # 이 이미 「**기간 중** 15분 최대수요가 난 날」 이라 **한 자리 안에서
            # 이름과 까닭이 갈려 있었다.** 12개월 미만 벌에는 해가 없다.
            label="기간 최대수요일",
            date=peak_day,
