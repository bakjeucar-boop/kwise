...
KNOWN_LIMITS: tuple[str, ...] = (
    ...
    "경제성DR 정산 단가는 산출하지 않습니다. 전력거래소가 지역별 SMP로 정산하는 몫과 "
    ...
    "(2023-01 ~ 2025-12)으로 계산하며 그 사실을 결과에 표시합니다. "
...
#
# **표 한 칸에 들어갈 길이로 적는다** (28세션 1-4). 「무엇이 없어서 못 냈나」 가
# 사유의 전부이고, 왜 지어내지 않는지·무엇을 넣어야 하는지는 그 자리의 입력
# 라벨·툴팁·보고서가 이미 말한다. 사유가 길면 표가 문단이 된다.
UNPRICED_REASONS: dict[str, str] = {
    "contract": "미산출 — 하한 규정 미확인",
...
#: 한 글자로 읽는다 (S247 결정 2).
ESS_NO_EXCESS = "어떤 목표에서도 초과 구간이 없어 곡선을 그리지 못했습니다."
...
def demand_split(peak_kw: float, billing_kw: float) -> bool:
    ...
    요금적용전력은 1 kW 단위로 접힌 값이라 관측 최대를 같은 단위로 접으면 갈리는가를 본다
    — 접힌 값이 정수이므로 「차가 0.5 kW 이상」 과 같은 말이다(접는 식 `round_kw` 는 요금
    쪽 세 자리에만 둔다 · `test_요금적용전력을_만드는_자리는_한_곳이다`). 앞서는 화면 · PPT ·
    Word 세 자리가 저마다 1% 문턱을 적어 대형 벌에서 52.93 kW 차이도 「같다」 로 접었다
    (S160 3절).
    """
    return round(peak_kw - billing_kw, 9) >= 0.5
