...
def money_axis_title(axes: Axes, text: str) -> None:
    ...
    # 같은 수를 두 꼴로 읽는다.
    #
    # **눈금끼리 갈리는 가장 적은 자리까지 적는다** (S251 · S234 ㄷ (나) 읽기 어려운 표기) —
    # 0자리로 못박아 억원 축이 「0 · 0 · 1 · 1 · 1 · 1 · 1 · 2」 를, 작은 차액 축이
    ...
    axes.yaxis.set_major_formatter(FuncFormatter(tick))
...
def daily_temperature_png(
    ...
    **평균 기준선을 함께 긋는다** (32세션 1절). 고온·저온이 부하를 미는 크기는
    ...
    것인지 눈금을 세어야 한다. 이름은 관측 기간 길이와 상관없이 「기간 평균」 이다 (S218).
