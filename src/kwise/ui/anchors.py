...
ANCHORS: tuple[ManualAnchor, ...] = (
    ...
    ManualAnchor(
        ...
        "근거, 연간 이용시간으로 선택요금을 가늠하는 방법. 화면에는 추정치와 확정만.",
    ),
    ManualAnchor(
        "improvement-summary",
        "개선 여지 요약",
    ...
    ManualAnchor(
        ...
        "부하 패턴 지표",
        "1단계 · 부하 패턴",
        "부하율·기저부하 비율·주말 비율·무인시간 부하의 정의와 읽는 법.",
    ),
    ManualAnchor(
        "peak-profile",
        "피크 특성과 상위 100구간",
    ...
    ManualAnchor(
        ...
        "7.1 선택요금 전환",
        "2단계 · 선택요금 전환 카드",
        "선택Ⅰ·Ⅱ·Ⅲ 의 설계 의도, 같은 계약종별·전압 안에서만 견주는 이유, "
        "전 조합을 다시 계산하는 이유.",
    ),
    ManualAnchor(
        "measure-contract",
        "7.2 계약전력 조정",
        "2단계 · 계약전력 조정 카드",
        "기본요금 기준(갑=계약전력·을=요금적용전력), 하한 비율 30%·교육용(을) "
        "15% 특례, 초과사용부가금과 위약 구조.",
    ),
    ManualAnchor(
        "measure-dr",
        "7.3 경제성DR",
    ...
    ManualAnchor(
        ...
        "7.7 잉여 활용",
        "2단계 · 잉여 활용 카드",
        "상계거래와 외부 구매의 제도 차이, 자격요건을 판정하지 않는 이유, "
        "외부 단가를 지어내지 않는 이유.",
    ),
    ManualAnchor(
        "weather-source",
    ...
    ManualAnchor(
        ...
        "조합 비교",
        "3단계 · 조합 비교",
        "조합마다 요금을 다시 계산하는 이유(단순 합산이 틀리는 구조), 조합 "
        "확실성이 가장 낮은 구성 요소를 따르는 이유.",
    ),
    ManualAnchor(
        "certainty",
        "확실성 등급",
        "3단계 · 조합 비교",
        "높음/중간/중간~낮음의 판정 근거와 수단별 배정. 화면에는 등급만 둔다.",
    ),
    ManualAnchor(
        "sensitivity",
        "감도 (첨예도 3종)",
        "3단계 · 감도",
        "평탄형·기준·첨예형의 정의와 총량 보존, **이름이 좋고 나쁨을 뜻하지 "
        "않는다**는 점, 상한 시나리오가 부하 형태에 따라 갈리는 이유, 요금제·"
        "계약전력·역률에 감도를 쓰지 않는 이유. 화면에는 범위 숫자만 둔다.",
    ),
    ManualAnchor(
        "payback",
        "회수기간",
