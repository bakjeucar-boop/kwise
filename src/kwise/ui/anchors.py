...
ANCHORS: tuple[ManualAnchor, ...] = (
    ...
    ManualAnchor(
        ...
        "1단계가 내는 것과 내지 않는 것",
        "1단계 · 머리말",
        "1단계는 진단만 하고 금액은 2단계에서 낸다. 옛 '투자 없이 가능한 "
    ...
    ManualAnchor(
        ...
        "Excel 산출물",
        "3단계 · 내려받기",
        "13시트의 구성과 읽는 순서, 감도 시트와 감도 상세의 분리, 파일명 접미사 규약.",
    ),
    ...
    ManualAnchor(
        ...
        "기준 데이터 관리",
        "기준 데이터 화면",
        "법령 유래와 판단값을 가르는 이유, 코드에 기본값을 두지 않는 원칙, 엑셀 왕복 보조 경로.",
    ),
    ManualAnchor(
        "rules-restore",
        "원복 세 경로",
