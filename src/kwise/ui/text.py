...
#
# **그림 툴팁은 기본요금이 무엇에 매이는지 말하지 않는다** (S170 2절). 이 상수는
# 갈래(피크형 · 하한형 · 계약형)를 모른다 — 「기본요금은 이 선으로 매겨집니다」
# 같은 말은 계약형 넷 · 하한형 셋에서 거짓이었다. 기준은 요금 엔진의 요금적용전력
# 안내가 갈래마다 따로 낸다.
CHART_TIPS: dict[str, str] = {
    "chart.monthly_peak": (
        "달마다 기록된 최대수요입니다. 붉은 점선이 요금적용전력입니다.\n\n"
        "한 달만 유난히 솟아 있으면 그 달의 몇십 분이 1년 최대수요를 정한 것입니다."
    ),
    "chart.top_hour": (
        # **「기간」 이다** (S213) — 그림이 보는 것은 올린 자료의 기간이다.
        "기간 최대수요 상위 100개 구간이 하루 중 언제 발생했는지 보여줍니다.\n\n"
    ...
    "chart.monthly_charge": (
        ...
        "보여줍니다.\n\n"
        "밑단(기본요금)은 대개 같은 높이로 이어지고, 그 위 "
        "계시별 세 조각이 계절과 조업에 따라 움직입니다."
    ),
    "chart.band": (
        "계시별 시간대(경부하·중간부하·최대부하)에 사용량이 어떻게 나뉘는지 계절마다 "
        "보여줍니다.\n\n"
        "최대부하 비중이 크면 단가가 낮은 시간대로 옮길 여지를 먼저 봅니다. 비중은 "
        "그 계절 안에서 다시 잽니다."
    ),
    "chart.tariff_option": (
    ...
    "chart.power_triangle": (
        ...
        "**각이 좁아질수록 역률이 좋습니다.** 삼각형이 둘이면 그 각도 차가 곧 "
        "역률 개선 폭입니다."
    ),
    ...
    "chart.solar_annual": (
        "날짜별 태양광 발전량입니다.\n\n여름에 높고 겨울에 낮은 계절 굴곡이 보입니다."
    ),
    "chart.solar_day": (
        "대표일의 피크 앞뒤 시간대입니다. 파란 두 선은 원래 부하와 태양광 적용 후 "
        "부하이고, 그 사이 초록이 저감분입니다.\n\n"
        "**피크 시각에 초록이 두꺼워야** 최대수요가 내려갑니다. 발전이 많아도 피크와 "
        "어긋나면 전력량요금만 줄어듭니다."
    ),
    "chart.ess_day": (
