...
#: 7.1 카드 본문에서 내리는 사실 셋 (27세션 4-3·4-4 · 64세션 4절).
#:
...
#:     요금적용전력 규칙 **1단계 「현재 요금 구조」 가 낸다** (64세션 4절). 제68조
#:                     한 문장이 두 자리에 글자까지 같이 떠 있었다. 규칙은
#:                     요금제를 바꿔도 그대로라 **여기서 요금제를 고르는 데
#:                     쓰이지 않는다.** 1단계에 그대로 있으므로 지워지는 설명이
#:                     없다 — 산출물에도 그대로 실린다
#:
...
def _ess(
    ...
    # 배제 사유는 제품 규격 하나이고 회수기간은 사유로 쓰지 않는다 (63세션).
