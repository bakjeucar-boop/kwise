...
_PF_LEAD = "역률 개선 설비를 조정해 역률을 높이면 기본요금이 감액됩니다. "
_PF_RULE = (
    "지상역률 92%를 넘으면 매 1%당 기본요금의 0.2%를 깎아주고, 97%까지 받을 수 "
    "있습니다. 미달하면 반대로 추가요금이 붙습니다."
)
NO_HEADROOM_OVERVIEW = _PF_RULE
"""여지가 없는 역률 카드(「없음」 판정)의 개요 — 거짓이 되는 앞머리를 뺐다 (S245 결정 3)."""

# DR 캡션의 앞 문장 「감축 가능량까지만 냅니다」 는 단가 미입력 판에서만 참이다 —
# 단가를 넣은 판은 뒤 문장만 세운다 (S245 결정 2 · S207).
DR_PRICED_HEADLINE = "정산 단가는 사업자 상담이 필요합니다."
...
_OVERVIEW: dict[str, str] = {
    ...
    # **개요는 기본요금 갈래를 가리지 않는 말만 쓴다** (S170 2절). 「최대수요와 하한
    # 중 큰 쪽」 · 「기본요금이 줄고」 · 「기본요금도 함께 줄어듭니다」 는 기본요금이
    # 피크에 매이는 벌에서만 참이었다 — 계약형(제68조 ②) 넷 · 하한이 전 달에
    # 걸린 셋에서 거짓이었다. 이 상수는 갈래를 모른다.
    "contract": (
        "기본요금이 계약전력이나 그 하한으로 매겨진 달은 계약전력을 낮추면 기본요금이 줄어듭니다."
    ),
    ...
    # 앞머리 「역률을 높이면 기본요금이 감액됩니다」 는 역률이 감액 상한 이상인 벌(「없음」
    # 판정)에서 거짓이라 그 벌에서는 세우지 않는다 — :data:`NO_HEADROOM_OVERVIEW`
    # (S245 결정 3 · S154). 그 개요는 40자 하한의 예외다(마-35 · 사람이 정했다).
    "power_factor": _PF_LEAD + _PF_RULE,
    "solar": (
        "옥상 등 유휴 공간에 발전 설비를 설치해 사용량을 줄입니다. 전력량요금이 직접 "
        "줄고, 발전 시간대와 피크 시각이 겹치면 최대수요도 함께 내려갑니다."
    ),
    "ess": (
        "배터리에 값싼 시간대 전기를 충전했다가 피크 시각에 방전해 최대수요를 "
        "낮춥니다. 시간대 단가차익도 생깁니다. 필요 용량이 목표에 "
        "매우 민감합니다."
    ),
}
...
_SCREEN: dict[str, tuple[str, str]] = {
    ...
    "contract": (
        ...
        # 「요금적용전력 하한에 걸려 있는지」 는 계약형(하한을 안 건다)에서 거짓이었다
        # (S170 2절).
        "계약전력을 얼마까지 낮출 수 있는지 봅니다.",
    ),
    "demand_response": (
        "measure-dr",
        "감축 가능량까지만 냅니다. " + DR_PRICED_HEADLINE,
    ),
    "power_factor": (
        "measure-power-factor",
        "현재 역률과 목표 역률로 각각 요금을 다시 계산합니다.",
    ),
