...
_OVERVIEW: dict[str, str] = {
    ...
    # **S207 2절이 여기서 멈췄다 — 기준은 판정을 주는데 울타리를 넘는다.**
    # 앞머리 「역률을 높이면 기본요금이 감액됩니다」 는 역률 100 세 벌에서 거짓이고
    # 뒤 두 문장은 제도 설명이라 위 주석의 규칙대로 매뉴얼 4.4 로 갈 자리다(그 표가
    # 92% 기준·매 1%당 0.2%·감액 상한 97%·추가 하한 60% 를 이미 다 적는다).
    # 그런데 **기준을 대고 남는 말이 「기본요금은 지상역률로 조정됩니다.」 18자**라
    # `test_ui.py::test_모든_카드에_개요가_있다` 의 40자 하한(14세션 「두세 줄」)에
    # 걸린다 — 그 하한은 카드 여섯 공용이라 이 판이 안 건드렸다. 값은 S207 1-4.
    "power_factor": (
