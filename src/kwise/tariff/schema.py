...
class OptionRates:
    ...
    Attributes:
        effective_date: **이 선택요금**의 시행일. 종별 시행일과 다를 수 있다 —
            ...
            (스키마 0.3). **꼴이 둘이다** — 약관이 날로 정하면 ``2026-06-01``,
            요금월로 정하면 ``2026-12`` 다. 요금월을 날로 적으면 없는 사실을
            적는 것이다(검침 기간이 달을 걸친다).
            **종별 값으로 채우는 폴백을 두지 않는다** — 없으면
...
class ContractType:
    ...
    Attributes:
        ...
        contract_floor_ratio: 요금적용전력의 계약전력 대비 하한. **요금적용전력을
            쓰는 종별은 전부 30% 다** (제68조 ①. 15% 는 별표4 8. 의 신청 특례라
            종별 속성이 아니다). 확인되지 않은 종별은 ``null`` 로 두면
