...
원문 요약 (정본은 ``data\\source\\2026-06-01_기본공급약관시행세칙(합본).pdf``
93·95쪽. S153 3절에 그 실물에서 다시 읽었다 — 앞서는
``2024-10-24_기본공급약관.pdf`` 를 적고 있었다.)

    제41조   고객은 전체 사용설비의 역률을 **지상역률 92% 이상**으로 유지한다.
    제42조 ① 무효전력계 미설치 고객의 역률은 **지상역률 92%로 본다.**
           ② 설치 고객은 **30분 단위 누적 계량값**으로 계산한다.
             **단서** — 저압 고객과 원격검침이 안 되는 고객은
             **1개월간 누적 계량값**으로 계산한다.
    제43조 ① 대상은 무효전력계가 설치된 고객 가운데 다음 둘이다.
             **1호** 저압으로 공급받는 **계약전력 20kW 이상**의 일반용·산업용·
                    농사용·임시전력 (**교육용은 없다**)
             **2호** 고압 이상으로 공급받는 일반용·교육용·산업용·농사용·임시전력
             — **저압도 대상이다.** 앞서 이 자리가 1호를 통째로 빠뜨리고
               「고압 이상」 이라고만 적고 있었다 (S152 가 값으로 봤다).
               ``small-ind-a1`` 처럼 저압 계약전력 75kW 인 벌이 여기 든다.
           ② 1호 08~22시  **지상역률 기준 92%**
                          미달 시 매 1%당 기본요금의 0.2% **추가** (역률 60%까지)
                          초과 시 매 1%당 기본요금의 0.2% **감액** (역률 97%까지)
                 **나목**  30분 단위 역률이 지상 60% 미달이면 60%로,
                          **지상 97% 초과이면 97%로 간주**하여 평균한다.
                          — 역률 100% 를 넣으면 이 단서로 **97 로 접힌다.**
