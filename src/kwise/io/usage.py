...
def _build_warnings(
    *,
    ...
    if period_days < 365:
        ...
        # **반올림**이라 용인 자료의 364.9895833… 이 「365일」 로 찍혀
        # **문장이 제 말을 부정했다.** 내림으로 바꾸면 이번에는 같은 장의
        # 「분석 기간 122일」 과 「121일」 로 갈린다 — **수를 두 번 적는 것이
        # 병이다.** 일수는 기간을 적는 자리가 이미 말하므로 여기서는 뺀다.
        messages.append("분석 기간이 12개월 미만입니다. 연간 환산 시 주의가 필요합니다.")
    if missing_rows:
