...
9. **전체 시험은 앞단에서 상한을 걸고 돌린다** (49세션). 앞단이면 결과를 그
   ...
   **`-n auto` 를 유지한다 — 메모리를 재고 정했다** (67세션). 43세션이 「여유
   ...
   - **43세션 말이 맞다. 오히려 더 준다** — 가용이 2,440 → **671 MB**(72%)까지
     떨어진다. `-n 4` 보다 최대 RSS 가 **485 MB(+27%)** 많다.
   - **그런데 그것이 기준이 아니다.** 기준은 **스왑이나 실패로 이어지느냐**다.
     하드폴트가 402초에 **27,202쪽(초당 68쪽)** 뿐이라 **스왑이 아니고**,
     1,496건이 두 판 다 통과했다. **여유가 줄어도 남으면 문제가 아니다.**
   - 얻는 것은 **179초(−31%)** 다. 그래서 유지한다.
   - **다만 커밋이 최고 12,383 / 13,429 MB (92%) 까지 간다.** 이 PC 에서
     시험이 도는 동안 **무거운 것을 함께 띄우지 않는다.**
