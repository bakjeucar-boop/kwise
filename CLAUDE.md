...
9. **전체 시험은 앞단에서 상한을 걸고 돌린다** (49세션). 앞단이면 결과를 그
   ...
   **소요는 PC 를 빼고 적지 않는다.** 두 대의 사양이 다르다.
   ...
   | PC | 실측 |
   |---|---|
   | **2번** (AMD Ryzen 5 4500U · 6코어 6스레드) | 1,493건 **9분 22초** (65세션) · 앞서 1,491건 8분 16초 (64세션) |
   | **1번** | **모름.** 재면 이 칸을 채운다 |
   ...
   **갈림길 — 앞단 전체 실행이 8분을 넘는 PC 에서는 갈래별로 나눠 돌린다**
   (`PROCEED.md` 「pytest 분할 실행」 의 갈래 넷). **2번 PC 는 이미 넘었다** —
   9분 22초이고 Bash 상한 10분까지 **38초**밖에 없다. 넘긴 뒤에 판단하면 늦다.
   갈래는 넷 다 2분 안쪽이라 상한에 한참 못 미친다.
