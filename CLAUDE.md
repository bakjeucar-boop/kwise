...
9. **전체 시험은 앞단에서 상한을 걸고 돌린다** (49세션). 앞단이면 결과를 그
   ...
   **소요는 PC 를 빼고 적지 않는다.** 두 대의 사양이 다르다.
   ...
   | PC | `-n auto` 실측 |
   |---|---|
   | **2번** (AMD Ryzen 5 4500U · 6코어 6스레드) | `auto` → **6** · 1,493건 **7분 45초** (66세션). `-n 4` 로는 9분 22초였다 (65세션) |
   | **1번** | **모름.** 재면 이 칸을 채운다 — `auto` 가 고른 수도 함께 적는다 |
   ...
   **갈림길 — 앞단 전체 실행이 8분을 넘는 PC 에서는 갈래별로 나눠 돌린다**
   ...
   7분 45초라 다시 안쪽으로 들어왔다** — Bash 상한 10분까지 2분 15초다.
   `-n 4` 시절에는 38초였다. 넘긴 뒤에 판단하면 늦으므로 **잴 때마다 본다.**
