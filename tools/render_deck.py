...
CASES: tuple[Case, ...] = (
    ...
    Case(
        key="small-ind-a1",
        title="용인 소규모 · 산업용(갑)Ⅰ 저압 · 계약전력 290 kW",
        csv=YONGIN_XLSX,
        ...
        # 저압 행은 그 단서를 안 타는 본래 자리다 — 벌 이름이 단순해진다.
        #
        # **290 kW 는 저압으로 성립하는 수다** — 약관 제23조 ② 가 「1,000kW
        # 미만까지 저압으로 공급시에는 계약전력이 500kW 미만이어야 한다」 고
        # 적고, 제59조 ③ 이 「계약전력 300kW 이상 500kW 미만 고객이 저압으로
        # 공급받는 경우에는 산업용전력(갑) 저압전력을 적용한다」 로 되받는다.
        # 종별 경계(제59조 ② 1호 · 4kW 이상 300kW 미만)도 안쪽이다.
        #
        # **`small-a2` 에서 종별과 전압을 함께 갈았다** — 저압 행의 선택요금은
        # `single` 하나뿐이라 고압의 Ⅰ·Ⅱ 를 그대로 쓸 수 없다. 기본요금이
        # `contract`(계약전력) 기준으로 바뀌는 유일한 용인 벌이다.
        contract_kw=290.0,
        area_m2=1_000.0,
        building_name="용인 소규모 공장(산업용 갑Ⅰ 저압)",
        sigungu="경기도/용인시",
    ),
    Case(
        key="small-ind-a2",
