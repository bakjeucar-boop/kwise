...
#: 계약전력 75 kW · 최대수요전력 32 · 역률 100 넷이 청구서 값이다.
#:
#: **회귀 케이스가 아니다** — 기간이 122일(2026-05-09~09-08)이라 12개월 환산이
#: 안 서고 로더가 「12개월 미만」 경고를 낸다. 덱에만 쓴다.
...
CASES: tuple[Case, ...] = (
    ...
    Case(
        ...
        # 온 것이 값으로 보인다.
        #
        # 85% 는 **기준 92%**(제41조) 아래라 추가 부과 쪽이다 —
        # 미달 7%p × 0.2%/%p(제43조 ②) = **기본요금의 1.4%** 가 붙는다.
        # **90% 가 아니다** — 기준·간주가 둘 다 92% 이고 90% 는 이 산식에 없다
        # (`data\rules_kr.json` 의 `power_factor.lagging_standard_pct`).
        # 하한은 60% 라 그 아래는 추가가 6.4% 로 멈춘다.
    ...
    Case(
        # **S158 이 실물에서 본 자리들의 조건이 함께 서는 유일한 벌이다**
        # (S159 1절). 사용자가 뽑은 산출물 셋(화면·Excel·PPT)에서 열여덟이
        # 나왔는데 **그 조건이 함께 서는 벌이 저장소에 하나도 없어** 값을
        # 저장소에서 다시 잴 자리가 없었다 — `small-a2` 는 역률이 간주 92 라
        # 역률 안내가 안 서고 잉여가 기본(출력제어)이라 0원이며, 잉여 벌
        # `small-b-sell` 은 **외부 판매**라 상계 몫이 다른 식으로 잡힌다.
        #
        # `small-a2` 에서 **셋만 갈았다** — 청구서를 재현하는 그 벌은
        # 그대로 두고 조건만 옮겨 세운다.
        #
        #     역률   간주 92 → 주간 지상 100 (감액 상한 위 · 개선 여지 0)
        #     면적   1,000 m² → 400 m² (밀도 보통 → **32.0 kWp**)
        #     잉여   기본(출력제어) → 상계거래(한전)
        #
        # **DR 단가·SMP·외부 판매 단가·투자비 필드를 더하지 않았다** — 이
        # 판이 보는 넷 가운데 어느 것도 DR 을 안 쓰고, 회수기간은 기존
        # 필드로 이미 선다.
        key="small-a2-pf100-offset",
        title="용인 소규모 · 일반용(갑)Ⅱ 고압A 선택Ⅱ · 역률 100 · 태양광 32 kWp · 잉여 상계",
        csv=YONGIN_XLSX,
        contract_type="general_a_2",
        voltage="high_a",
        option="II",
        contract_kw=290.0,
        area_m2=400.0,
        building_name="용인 소규모 건물(역률 100 · 상계)",
        sigungu="경기도/용인시",
        surplus_use="상계거래(한전)",
    ...
    Case(
        key="large-edu-b",
        title="대형 · 교육용(을) 고압A 선택Ⅰ",
        csv=LARGE_CSV,
        contract_type="education_b",
        voltage="high_a",
        option="I",
        # **덱 벌이 하나도 없던 종별 넷 가운데 하나다** (S137 3절 · 91세션 등재).
        # 그 종별에서 덱·Excel·Word 가 어떻게 그려지는지 아무도 본 적이 없었다 —
        # **뜨지 않는 갈래는 없는 갈래와 같다.**
        #
        # **`large-b` 에서 종별 한 칸만 갈았다** — 자료·전압·선택요금·계약전력·
        # 면적·지역·역률이 다 같다. S132 의 `large-b-pf85` 와 같은 방식이라
        # 둘을 나란히 뽑으면 갈린 값이 종별에서만 온 것이 값으로 보인다.
        #
        # **6,000 kW 는 성립하는 수다** — 교육용전력(을)은 계약전력 1,000 kW
        # 이상이고(기본공급약관 제58조 ② 2호) 고압A 는 표준전압 3,300V 이상
        # 66,000V 이하 고객이라 위쪽 한도가 없다. 선택요금은 Ⅰ·Ⅱ 둘뿐이라
        # `large-b`(Ⅰ·Ⅱ·Ⅲ)와 후보 수가 다르다.
        contract_kw=6_000.0,
        area_m2=20_000.0,
        building_name="대형 학교(교육용 을)",
        sigungu=REGION,
    ),
    Case(
        key="small-ind-a1",
        title="소규모 공장 · 산업용(갑)Ⅰ 저압 · 계약전력 75 kW",
        csv=IND_LOW_XLSX,
        contract_type="industrial_a_1",
        voltage="low",
        option="single",
        # **S153 에 합성을 실측으로 갈아 끼웠다.** S150 이 세울 때는 용인 부하
        # (`YONGIN_XLSX` · 290 kW)에 산업용 저압을 씌운 **합성**이었다. S152 에
        # 이 종별의 실측이 들어왔고 **실물 청구서가 따라왔으므로**(계약종별 ·
        # 계약전력 75 kW · 역률 100) 실측이 이긴다.
        #
        # **저압으로 세운다** (S150 3절). 갑Ⅰ 고압 행은 제59조 ⑤ 가 「갑 고압
        # 고객은 갑Ⅱ 를 적용한다」 고 못을 박아 **저압계량 예외 경로**이지만,
        # 저압 행은 그 단서를 안 타는 본래 자리다.
        #
        # **75 kW 는 저압으로 성립하는 수다** — 약관 제23조 ② 가 「1,000kW
        # 미만까지 저압으로 공급시에는 계약전력이 500kW 미만이어야 한다」 고
        # 적고, 종별 경계(제59조 ② 1호 · 4kW 이상 300kW 미만)도 안쪽이다.
        # 계약전력이 290 → 75 kW 로 줄어도 **두 조문 다 그대로 안쪽이다.**
        #
        # **저압 행의 선택요금은 `single` 하나뿐이다.** 기본요금이
        # `contract`(계약전력) 기준으로 바뀌는 유일한 덱 벌이다.
        #
        # **회귀 벌 R3 과의 짝이 여기서 깨진다** (S153 1-2). S150 이 이 벌과
        # R3 을 같은 조건으로 세워 덱과 회귀를 맞대 볼 수 있게 했는데, R3 은
        # 용인 자료(290 kW)에 남고 이 벌만 실측(75 kW)으로 간다. R3 을 따라
        # 갈지 못한다 — 122일이라 12개월 환산이 안 서서 회귀에 못 올린다.
        # 종별·전압·선택요금 셋은 여전히 같으므로 **경로는 짝이고 값이 아니다.**
        # 미해결에 이름으로 올렸다.
        contract_kw=75.0,
        # **청구서의 역률 100 이다.** 이 갈래가 회귀에 한 번도 안 섰다 —
        ...
        # 갈기 전 값(경기도/용인시)을 그대로 두고 미해결에 올렸었다.
        # **값에 닿는 자리다** — 덱은 `ALL_MEASURES` 로 태양광을 켜므로 이
        # 좌표가 발전량을 정한다 (97세션 1절이 못을 박은 자리다). 갈아 보니
        # 80 kWp 에서 연 39,530.24 → **39,564.78 kWh**(+0.09%) · 자가소비율
        # 58.72 → **59.51%** · 잉여 16,319.76 → **16,018.82 kWh** · 태양광
        # 절감액 2,583,382.00 → **2,620,572.70원**(+1.44%) 로 갈렸다.
        sigungu="경상북도/구미시",
    ),
    Case(
        key="small-ind-a2",
    ...
    Case(
        ...
        option="I",
        # **덱 벌이 하나도 없던 종별 넷 가운데 하나다** (S137 3절 · 91세션 등재).
        # `large-edu-b` 와 같은 자리이고 **여기도 종별 한 칸만 갈았다.**
        #
        # **6,000 kW 는 성립하는 수다** — 산업용전력(을)은 계약전력 300 kW
        # 이상이고(기본공급약관 제59조 ② 2호) 고압A 는 표준전압 3,300V 이상
        # 66,000V 이하 고객이다. 산업용은 별표 2(적용대상 기준표)의 광업·제조업
        # 고객에게 적용하므로 **자료는 대형 실측이고 종별만 합성**이다 —
        # `small-edu-a` 가 소형 사무빌딩 자료로 학교를 세운 것과 같은 꼴이다.
        #
        # **남은 종별 둘은 못 세운다** — 산업용(갑)Ⅰ·(갑)Ⅱ 는 제59조 ② 1호가
        # 계약전력 300 kW **미만**으로 못을 박아 대형 정본 6,000 kW 가 범위
        # 밖이다. 후보는 용인 실측(290 kW)이다 — 소형 사무빌딩은 300 kW 라
        # 경계 위여서 갑에 못 쓴다.
        contract_kw=6_000.0,
        area_m2=20_000.0,
