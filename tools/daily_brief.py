...
def build() -> str:
    ...
    # **조각으로 갈라 앞 셋을 내지 않는다** (S146 1절). 요약은 ` · ` 로 갈린
    # 목록이 **아니라** 그런 목록을 품은 글이다 — 갈라서 앞 셋을 내면 둘째·셋째가
    # 「몫」(S144) · 「그룹 막대 조각 합(요금제와 −6,339,264원)」(S145) 처럼
    # **앞말 없는 토막**이 된다. :func:`split_top` 은 괄호만 세고 「」 는 안 세어
    # 인용 가운데서 끊기기도 했다 — S144 행이 「S139 갑Ⅰ 66,571,213원 에서
    # **닫는 따옴표도 말줄임표도 없이** 끝났다.
    #
    # **머리를 이어서 낸다.** 280자는 지금 세 줄이 먹던 자리와 같고, 자른 자리는
    # :func:`clip` 이 … 로 말한다 — 이 칸은 자르는 칸이 맞다 (68세션 2절).
    lines.extend(wrap(clip(strip_md(detail), 280), "  · ", "    "))
    lines.append("")
