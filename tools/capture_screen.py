...
SPOTS: tuple[Spot, ...] = (
    ...
    # **선택요금 그림은 요금제가 넷인 벌에서 본다** (S162 1절). 을 고압A 는 셋이고
    # 교육갑은 둘이라 축 이름이 모자라는 자리가 안 선다 — 갑Ⅱ 고압A 가 넷이다.
    Spot(
        key="선택요금",
        title="2단계 › 선택요금 전환 (일반용(갑)Ⅱ 고압A 선택Ⅱ · 290 kW — 요금제 넷)",
        # 지표 셋에 걸어야 **그림 둘이 한 화면에 들어온다** — 차액 막대는 그 바로
        # 아래이고 그룹 막대가 이어진다. 캡션에 걸면 그림이 위로 밀려 나간다.
        anchor="가장 유리한 요금제",
        wait_for="피크 특성",
        contract=YONGIN_A2,
        tab="2단계 · 개선 수단",
