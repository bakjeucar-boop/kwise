...
#:
#: **경계는 ASCII 다** (S156 3-3). `\w` 는 한글을 낱말 문자로 세므로
#: `(?<![\w…])` 로 두면 「…역률power_factor」 처럼 **한글에 바로 붙은 열쇠**를
#: 놓친다 — 열쇠가 한글 사이에 끼는 것이 바로 이 규칙이 잡으려는 모양이다.
#: `tests\test_artifact_words.py` 가 같은 정규식을 쓰는지 값으로 문다.
#:
#: **이 경계로도 못 잡는 것이 있다** — `single` 처럼 밑줄도 `=` 도 없는 열쇠는
#: 어떤 경계로도 안 걸린다. 그 갈래는 정규식이 아니라 `option_label` 이 막는다.
...
#: 별표 앞에 있어야 할 규정 이름. 앞서는 「기본공급약관시행세칙」 하나여서
#: 97세션이 「전력시장운영규칙」 을 더했고, **S156 이 세칙을 약관으로 줄였다** —
#: 약관 본문의 별표3 을 「기본공급약관 별표3 단서 3호」 로 **옳게** 적은 자료
#: 세 자리가 Excel·Word 에서 위반으로 잡혔다(S156 3-1 이 실물에서 값으로 봤다).
#: **줄여도 잃는 것이 없다** — 「기본공급약관시행세칙」 은 「기본공급약관」 을
#: 안에 담고 있어 부분 문자열로 함께 걸린다.
#: 규칙의 뜻은 그대로다: 별표 번호만 적지 않는다.
SCHEDULE_OWNERS = ("전력시장운영규칙", "기본공급약관")
