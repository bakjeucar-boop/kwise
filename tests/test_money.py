...
def test_단수_차이_조정은_다른_표에서_선_줄을_그대로_두고_나머지를_올린다() -> None:
    """**고정 줄** (S233 ㄱ) — 계약 표의 현재 기본요금은 청구 표에서 올린 값 그대로다.

    나머지가 가장 큰 줄은 현재 기본요금(900원)이지만 청구 표가 100,000원으로 적었으므로
    그 값을 쓰고, 셈은 역률 줄(300원)이 올라 맞춘다.
    """
    shown = money.balance_won(
        [100_900.0, 40_100.0, 5_300.0], 66_100.0, [1, -1, 1], [100_000.0, None, None]
    )
    assert shown == [100_000.0, 40_000.0, 6_000.0]
...
def test_두_합계의_차는_적힌_두_합계의_차다(current: float, best: float) -> None:
    """(S233 ㄴ) — 원값의 차를 절사하면 적힌 두 합계의 차와 1,000원 어긋난다."""
