...
#: ``docs\OPEN_ITEMS.md`` 의 갈래 절 제목 — `## 가 — 계산 결과 값이 틀린다 (7)`.
BRANCH_HEAD = re.compile(r"^## ([가-마]) —[^\n(]*\((\d+)\)\s*$", re.M)


def test_갈래_절_제목의_합이_미해결_건수와_같다() -> None:
    """**넷째 자리다 — 갈래 표의 합** (S161 0-2절).

    수를 세는 자리가 셋인데 못은 둘만 있었다. :data:`COUNTS` 의 「미해결 건수」 는
    ``미해결 N건`` 이라 적힌 자리(이 문서의 「지금 수」)를 물고,
    :func:`test_미해결_갈래의_머리말과_줄_수가_같다` 는 ``PROCEED.md`` 안에서
    머리말과 줄을 맞댄다 — **갈래 절 제목의 합은 어느 쪽도 안 봤다.**
    그 칸을 세 판(S147·S153·S157)이 안 갈고 지났고 S160 이 사람 눈으로 찾았다.

    **S161 이 세 수를 다시 재니 98 · 98 · 95 였다.** 셋째(줄 95)는 결함이 아니다 —
    ①-1 「청구서 4」 한 줄이 넷을 담아 머리말 7 에 줄이 4 인 것이고
    (68세션 2절) 위 못이 그 뭉침을 세어 98 로 맞춘다. **어긋남이 아니라 뭉침이다.**
